...
- (void)startAdvertisingData:(NSDictionary<CBUUID *, NSData *> *)serviceData
  ...
  dispatch_async(_queue, ^{
    ...
    // 28 byte limit. Since we have a 4-byte service UUID and the header for local name and service
    // UUID is 2 bytes each, that leaves us with a maximum of 20 bytes for the local name.
    // (2 + 20) + (2 + 4) = 28
