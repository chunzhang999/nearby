...
class TestPresenceDeviceProvider : public PresenceDeviceProvider {
 public:
  TestPresenceDeviceProvider(
      ServiceController* service_controller,
      std::unique_ptr<ConnectionAuthenticator> connection_authenticator)
      : PresenceDeviceProvider(service_controller,
                               std::move(connection_authenticator)) {}
};
...
class PresenceDeviceProviderTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    auto connection_authenticator =
        std::make_unique<MockConnectionAuthenticator>();
    mock_connection_authenticator_ = connection_authenticator.get();
    provider_ = std::make_unique<TestPresenceDeviceProvider>(
        &mock_service_controller_, std::move(connection_authenticator));
    default_initiator_data_.shared_credential_hash = kSharedCredentialHash;
    default_initiator_data_.private_key_signature = kPrivateKeySignature;
    ON_CALL(*mock_connection_authenticator_, BuildSignedMessageAsInitiator)
        .WillByDefault(testing::Return(default_initiator_data_));
    auto key_pair_or_status = crypto::Ed25519Signer::CreateNewKeyPair();
    ASSERT_OK_AND_ASSIGN(key_pair_, key_pair_or_status);
  }

 protected:
  ConnectionAuthenticator::TwoWayInitiatorData default_initiator_data_;
...
TEST_F(PresenceDeviceProviderTest,
       AuthenticateAsInitiator_NoLocalPublicCredentials) {
  EXPECT_CALL(mock_service_controller_, GetLocalCredentials)
      .WillOnce([&](const CredentialSelector& credential_selector,
                    GetLocalCredentialsResultCallback callback) {
        std::vector<nearby::internal::LocalCredential> credentials;
...
TEST_F(PresenceDeviceProviderTest, AuthenticateAsInitiator_Success) {
  ...
  EXPECT_CALL(authentication_transport, ReadMessage).WillOnce([&]() {
    PresenceAuthenticationFrame authentication_frame;
    authentication_frame.set_private_key_signature(key_pair_.private_key);
    return authentication_frame.SerializeAsString();
  });
