...
TEST(ServiceControllerImplTest, GetLocalCredentials) {
  ...
  EXPECT_EQ(3u, private_credentials->size());
  EXPECT_EQ(kSecretId1, private_credentials->at(0).secret_id());
  EXPECT_EQ(kSecretId2, private_credentials->at(1).secret_id());
  EXPECT_EQ(kSecretId3, private_credentials->at(2).secret_id());
}
